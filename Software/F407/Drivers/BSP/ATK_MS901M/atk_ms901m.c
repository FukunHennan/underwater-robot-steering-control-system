 ...
 * @version     V1.0
 * @date        2026-04-14
 * @brief       水下智能转向系统 - ATK-MS901M 模块驱动代码
...
#define ATK_MS901M_FRAME_HEAD_UPLOAD_H      0x55    /* 上传数据帧头高字节 */
#define ATK_MS901M_FRAME_HEAD_ACK_H         0xAF    /* 应答帧头高字节 */
...
typedef struct
{
    uint8_t head_l;                                 /* 帧头低字节 */
    uint8_t head_h;                                 /* 帧头高字节 */
    uint8_t id;                                     /* 帧ID */
    uint8_t len;                                    /* 数据长度 */
    uint8_t dat[ATK_MS901M_FRAME_DAT_MAX_SIZE];     /* 数据内容 */
    uint8_t check_sum;                              /* 校验和 */
} atk_ms901m_frame_t;                               /* ATK-MS901M UART帧结构体 */

typedef enum
{
    wait_for_head_l = 0x00,                         /* 等待帧头低字节 */
    wait_for_head_h = 0x01,                         /* 等待帧头高字节 */
    wait_for_id     = 0x02,                         /* 等待帧ID */
    wait_for_len    = 0x04,                         /* 等待数据长度 */
    wait_for_dat    = 0x08,                         /* 等待数据内容 */
    wait_for_sum    = 0x16,                         /* 等待校验和 */
} atk_ms901m_handle_state_t;                        /* 帧处理状态机枚举 */
...
static struct
{
    uint8_t gyro;                                   /* 陀螺仪满量程配置 */
    uint8_t accelerometer;                          /* 加速度计满量程配置 */
} g_atk_ms901m_fsr;                                 /* ATK-MS901M满量程配置 */
 ...
 *              id     : 目标帧ID
 *              id_type: 目标ID类型
 *                       ATK_MS901M_FRAME_ID_TYPE_UPLOAD: ATK-MS901M上传数据ID
 *                       ATK_MS901M_FRAME_ID_TYPE_ACK   : ATK-MS901M应答ID
 *              timeout: 等待帧数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK     : 获取到了指定的帧
 *              ATK_MS901M_EINVAL  : 传入参数有问题
 *              ATK_MS901M_ETIMEOUT: 超时仍未获取到指定帧
 */
...
/**
 * @brief       通过ID读取ATK-MS901M的寄存器
 * @param       id     : 要读取的寄存器ID
 *              dat    : 读取到的数据
 *              timeout: 等待应答的超时时间，单位ms
 * @retval      0: 读取失败
 *              非0: 实际读取到的数据长度
 */
...
/**
 * @brief       通过ID写入ATK-MS901M的寄存器
 * @param       id : 要写入的寄存器ID
 *              len: 要写入的数据长度，1或2
 *              dat: 要写入的数据内容
 * @retval      ATK_MS901M_EOK   : 写入操作完成
 *              ATK_MS901M_EINVAL: 传入的len有问题
 */
...
/**
 * @brief       ATK-MS901M初始化
 * @param       baudrate: ATK-MS901M UART通信波特率
...
/**
 * @brief       获取ATK-MS901M的姿态数据
 * @param       attitude_dat: 姿态数据存放位置
 *              timeout     : 等待姿态数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 获取ATK-MS901M姿态数据成功
 *              ATK_MS901M_ERROR: 获取ATK-MS901M姿态数据失败
 */
...
/**
 * @brief       获取ATK-MS901M的四元数数据
 * @param       quaternion_dat: 四元数数据存放位置
 *              timeout       : 等待四元数数据的超时时间，单位ms
...
/**
 * @brief       获取ATK-MS901M的陀螺仪和加速度计数据
 * @param       gyro_dat         : 陀螺仪数据存放位置
 *              accelerometer_dat: 加速度计数据存放位置
 *              timeout          : 等待数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 获取ATK-MS901M陀螺仪和加速度计数据成功
 *              ATK_MS901M_ERROR: 获取ATK-MS901M陀螺仪和加速度计数据失败
 */
...
/**
 * @brief       获取ATK-MS901M的磁力计数据
 * @param       magnetometer_dat: 磁力计数据存放位置
 *              timeout         : 等待数据的超时时间，单位ms
...
/**
 * @brief       获取ATK-MS901M的气压计数据
 * @param       barometer_dat: 气压计数据存放位置
 *              timeout      : 等待数据的超时时间，单位ms
...
/**
 * @brief       获取ATK-MS901M的端口数据
 * @param       port_dat: 端口数据存放位置
 *              timeout : 等待数据的超时时间，单位ms
...
/**
 * @brief       获取ATK-MS901M LED状态
 * @param       state: LED状态存放位置
 *              timeout: 等待数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 获取ATK-MS901M LED状态成功
 *              ATK_MS901M_ERROR: 获取ATK-MS901M LED状态失败
 */
...
/**
 * @brief       设置ATK-MS901M LED状态
 * @param       state: LED状态
 *              timeout: 等待数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 设置ATK-MS901M LED状态成功
 *              ATK_MS901M_ERROR: 设置ATK-MS901M LED状态失败
 */
...
/**
 * @brief       获取ATK-MS901M端口模式
 * @param       port   : 目标端口
 *              mode   : 模式存放位置
 *              timeout: 等待数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 获取ATK-MS901M端口模式成功
 *              ATK_MS901M_ERROR: 获取ATK-MS901M端口模式失败
 */
...
/**
 * @brief       设置ATK-MS901M端口模式
 * @param       port   : 目标端口
 *              mode   : 目标模式
 *              timeout: 等待数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 设置ATK-MS901M端口模式成功
 *              ATK_MS901M_ERROR: 设置ATK-MS901M端口模式失败
 */
...
/**
 * @brief       获取ATK-MS901M端口PWM脉宽
 * @param       port   : 目标端口
 *              pulse  : 端口PWM脉宽存放位置，单位ns
 *              timeout: 等待数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 获取ATK-MS901M端口PWM脉宽成功
 *              ATK_MS901M_ERROR: 获取ATK-MS901M端口PWM脉宽失败
 */
...
/**
 * @brief       设置ATK-MS901M端口PWM脉宽
 * @param       port   : 目标端口
 *              pulse  : 端口PWM脉宽存放位置，单位ns
 *              timeout: 等待数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 设置ATK-MS901M端口PWM脉宽成功
 *              ATK_MS901M_ERROR: 设置ATK-MS901M端口PWM脉宽失败
 */
...
/**
 * @brief       获取ATK-MS901M端口PWM周期
 * @param       port   : 目标端口
 *              period : 端口PWM周期存放位置，单位ns
 *              timeout: 等待数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 获取ATK-MS901M端口PWM周期成功
 *              ATK_MS901M_ERROR: 获取ATK-MS901M端口PWM周期失败
 */
...
/**
 * @brief       设置ATK-MS901M端口PWM周期
 * @param       port   : 目标端口
 *              period : 端口PWM周期存放位置，单位ns
 *              timeout: 等待数据的超时时间，单位ms
 * @retval      ATK_MS901M_EOK  : 设置ATK-MS901M端口PWM周期成功
 *              ATK_MS901M_ERROR: 设置ATK-MS901M端口PWM周期失败
 */

...
#define ATK_MS901M_FRAME_HEAD_UPLOAD_H      0x55    /* ??¦Ë????????? */
#define ATK_MS901M_FRAME_HEAD_ACK_H         0xAF    /* ??¦Ë????? */
...
typedef enum
{
    wait_for_head_l = 0x00,                         /* ?????¦Ë?? */
    wait_for_head_h = 0x01,                         /* ?????¦Ë?? */
    wait_for_id     = 0x02,                         /* ????ID */
    wait_for_len    = 0x04,                         /* ?????????? */
    wait_for_dat    = 0x08,                         /* ??????? */
    wait_for_sum    = 0x16,                         /* ???§µ??? */
...
/**
 * @brief       ????ID§Õ??ATK-MS901M?????
 * @param       id : ?????????????ID
 *              len: ??§Õ??????????1??2??
 *              dat: ??§Õ???????
 * @retval      ATK_MS901M_EOK   : ?????§Õ????

...
### 寄存器映射（每个舵机 4 个 float 参数）

| 地址 | 名称 | 类型 | 默认值 | 说明 |
|------|------|------|--------|------|
| **舵机 1** | | | | |
| 0x00A0 | SERVO1_BASE | RW | 0.0 | 基础角度偏移 (deg, float) |
| 0x00A2 | SERVO1_K_ROLL | RW | 0.0 | Roll 补偿系数 (float) |
| 0x00A4 | SERVO1_K_PITCH | RW | 0.0 | Pitch 补偿系数 (float) |
| 0x00A6 | SERVO1_K_YAW | RW | 0.0 | Yaw 补偿系数 (float) |
| **舵机 2** | | | | |
| 0x00A8 | SERVO2_BASE | RW | 0.0 | 基础角度偏移 (deg, float) |
| 0x00AA | SERVO2_K_ROLL | RW | 0.0 | Roll 补偿系数 (float) |
| 0x00AC | SERVO2_K_PITCH | RW | 0.0 | Pitch 补偿系数 (float) |
| 0x00AE | SERVO2_K_YAW | RW | 0.0 | Yaw 补偿系数 (float) |
| **舵机 3-8** | (类似结构) | | | 地址依次递增 8 |

**数据格式**: IEEE 754 单精度浮点，大端字序 (AB CD)

### 舵机自动补偿使能标志 (0x00E0 - 0x00E7)

| 地址 | 名称 | 类型 | 默认值 | 说明 |
|------|------|------|--------|------|
| 0x00E0 | SERVO1_AUTO_EN | RW | 0 | 舵机1 自动补偿使能 (0=禁用, 1=启用) |
| 0x00E1 | SERVO2_AUTO_EN | RW | 0 | 舵机2 自动补偿使能 |
| 0x00E2 | SERVO3_AUTO_EN | RW | 0 | 舵机3 自动补偿使能 |
| 0x00E3 | SERVO4_AUTO_EN | RW | 0 | 舵机4 自动补偿使能 |
| 0x00E4 | SERVO5_AUTO_EN | RW | 0 | 舵机5 自动补偿使能 |
| 0x00E5 | SERVO6_AUTO_EN | RW | 0 | 舵机6 自动补偿使能 |
| 0x00E6 | SERVO7_AUTO_EN | RW | 0 | 舵机7 自动补偿使能 |
| 0x00E7 | SERVO8_AUTO_EN | RW | 0 | 舵机8 自动补偿使能 |

### 使用示例

**场景**: 让舵机1 根据 Roll 角自动调整 ±30° 范围

1. 设置补偿系数：
   ```
   SERVO1_K_ROLL = 1.0    // Roll 每变化 1°，舵机角度变化 1°
   SERVO1_BASE = 0.0      // 基础角度为 0°
   SERVO1_AUTO_EN = 1     // 启用自动补偿
   ```

2. 当 Roll = 15° 时：
   ```
   目标角度 = 0.0 + 1.0 × 15 + 0.0 × Pitch + 0.0 × Yaw = 15°
   PWM = 1500 + 15 × 10 = 1650 μs
   ```

3. 当 Roll = -20° 时：
   ```
   目标角度 = 0.0 + 1.0 × (-20) = -20°
   PWM = 1500 + (-20) × 10 = 1300 μs
   ```

**高级应用**: 多轴联动补偿
```
// 让舵机同时响应 Roll 和 Pitch
SERVO1_K_ROLL = 0.8
SERVO1_K_PITCH = 0.5
SERVO1_K_YAW = 0.0

// 当 Roll=10°, Pitch=5° 时：
目标角度 = 0.8×10 + 0.5×5 = 10.5°
```
